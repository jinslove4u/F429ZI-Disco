/******************************************************************************/
/*                                                                            */
/*  PACMAN GAME FOR ARDUINO STM32F429-disco Mod by JINsMac                    */
/*                                                                            */
/******************************************************************************/
...
#define USE_TFT  // For VGA leave uncommented
...
#ifdef USE_TFT
// TFT library by stimmer, v0.404 (4/4/2013) http://stimmer.github.io/DueVGA/ 
#include "TFT.h"
#endif
...
public: 
    ...
    void Draw(uint16_t x, uint16_t y, bool sprites)   // 스프라이트 그리기 
    {
...
//------------------------------------------------------------------------------
       ...
       for (byte tmpY=0;tmpY<8;tmpY++) {
            ...
            for (byte tmpX=0;tmpX<8;tmpX++) {
               ...
               if (tmpX==7 && width==1 ) {TFT.drawPixel(x+tmpX, y+tmpY, color); color=next_color; width=0; }
               else if (tmpX==7 && width>1 ) { TFT.drawHLine( y+tmpY,x+tmpX-width+1, x+tmpX+1, color); color=next_color; width=0; }
               else if (color!=next_color && width==1) { TFT.drawPixel(x+tmpX, y+tmpY, color);  color=next_color; width=0; }
               else if (color!=next_color && width>1) { TFT.drawHLine( y+tmpY,x+tmpX-width+1, x+tmpX+1, color);  color=next_color; width=0; }
               width++;
...
//------------------------------------------------------------------------------
    ...
    void Init()  //초기화 작업 
    {
       ...
       const byte* s = _initSprites;
       for (int16_t i = 0; i < 5; i++)
           _sprites[i].Init(s + i*5);   // 스프라이트 초기화 
...
void setup(){
  ...
  // START KEYPAD LOOP
  Scheduler.startLoop(KeyPadLoop);  // 스케줄러는 타이밍 처리용 

//------------------------------------------------------------------------------
#ifdef USE_TFT
  // SETUP TFT OUTPUT
  TFT.begin(240,320,TFT_COLOUR);  // 게임화면은 240x320 화면 
#endif
//------------------------------------------------------------------------------
#ifdef USE_ILI          // 시리얼LCD출력용 여기서는 사용안함. 
  // SETUP TFT LCD
  tft.begin();
  delay(100);
