...
void Tft::drawText(char *text, int x, int y, int c, int b, int dir)
{
  uint8_t t;
  while(t=(uint8_t)*text++){
    for(int j=0;j<8;j++)for(int i=0;i<8;i++){
      ...
      case -1:
      case 0:     
	  if(_tft_font8x8[8*t+j]&(128>>i))drawPixel(x+i,y+j,c);
	  else if(b!= -256) drawPixel(x+i,y+j,b);
	  break;
      case 1:
	  if(_tft_font8x8[8*t+j]&(128>>i))drawPixel(x+j,y-i,c);
...
void Tft::moveCursor(int column, int line)
{
  tx=column;if(tx<0)tx=0;if(tx>=tww)tx=tww-1;
  ty=line;if(ty<0)ty=0;if(ty>=twh)ty=twh-1;
}

void Tft::setPrintWindow(int left, int top, int width, int height){
...
void Tft::clearPrintWindow(){
    fillRect(twx*8,twy*8,(twx+tww)*8-1,(twy+twh)*8-1,paper);
    tx=0;ty=0;
}
void Tft::scrollPrintWindow()
{
...
size_t Tft::write(uint8_t c){
  ...
  if(mode==TFT_MONO){
    ...
    for(int j=0;j<8;j++){
      *a=_tft_font8x8[8*c+j]^((ink&1)?0:255);      
      a+=pw*2;
    }
  }
  else if(mode&TFT_COLOUR){
    ...
    for(int j=0;j<8;j++){
      for(int i=0;i<8;i++){
	a[i]=(_tft_font8x8[8*c+j]&(128>>i))?ink:paper;      
      }
      a+=cw;
    }
  }
