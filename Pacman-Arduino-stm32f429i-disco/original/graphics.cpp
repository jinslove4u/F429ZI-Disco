...
void Tft::clear(int c){
    if(mode==TFT_MONO){
	for(int y=0;y<ysize;y++){
	      memset(pb+y*pw,(c&1)?0xff:0,xsize/8);
	}
    }
    else if(mode&TFT_COLOUR){
      memset(cb,c,cbsize);  
    }
  
}

void Tft::drawPixel(int x, int y, int c)
{
...
void Tft::drawLine(int x0, int y0, int x1, int y1,int c)
{
...
void Tft::drawHLine(int y,int x0, int x1, int col)
{
  for(int i=x0;i<=x1;i++)drawPixel(i,y,col);
}

void Tft::drawLinex(int x0, int y0, int x1, int y1,int c) 
...
void Tft::drawTri(int x0,int y0,int x1,int y1,int x2,int y2,int col)
{ 
...
void Tft::fillTri(int x0,int y0,int x1,int y1,int x2,int y2,int col)
...
void Tft::drawRect(int x0, int y0, int x1, int y1, int col)
{
...
void Tft::fillRect(int x0, int y0, int x1, int y1, int col){
...
// by Zingl Alois
void Tft::drawCircle(int xm, int ym, int r, int col){
...
void Tft::fillCircle(int xm, int ym, int r, int col){
   short xmin[ysize],xmax[ysize];
   for(int i=0;i<ysize;i++){xmin[i]=xsize;xmax[i]= -1;}
   int x = -r, y = 0, err = 2-2*r;                /* bottom left to top right */
   do {                                          
...
void Tft::drawEllipse(int x0, int y0, int x1, int y1, int col)
...
void Tft::fillEllipse(int x0, int y0, int x1, int y1, int col)
{                              /* rectangular parameter enclosing the ellipse */
   long a = abs(x1-x0), b = abs(y1-y0), b1 = b&1;                 /* diameter */
   double dx = 4*(1.0-a)*b*b, dy = 4*(b1+1)*a*a;           /* error increment */
   double err = dx+dy+b1*a*a, e2;                          /* error of 1.step */
...
void Tft::scroll(int x, int y, int w, int h, int dx, int dy,int col){
  if(mode&TFT_COLOUR){
    if(dy<=0){
      if(dx<=0){      
