...
#define adj_width  288
#define adj_height 240
#define START_LIFES 2
#define START_LEVEL 1

#define Start_B PA0 // f429-disco button
...
#define C16(_rr,_gg,_bb) ((_rr & 0xF8) << 8) | ((_gg & 0xFC) << 3) | (_bb >> 3)
            ...
            for (byte tmpX=0;tmpX<8;tmpX++) {
               ...
               //if ((color!=next_color && width>=1) || tmpX==7 ) {
                 ...
                 ///drawPixel_cont(x+tmpX, y+tmpY, color);
                  //VGA.drawHLine( y+tmpY,x+tmpX-width+1, x+tmpX+1 + width, color);
                  //tft.drawFastHLine( y+tmpY,x+tmpX-width+1, x+tmpX+1 + width, color);
                 tft.drawPixel(x+tmpX, y+tmpY, color>>8 | color<<8);
                 color=next_color;
                 width=0;
               //}
               width++;
